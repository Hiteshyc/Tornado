...
import {
  ...
  Plus, X, AlertTriangle, Send,
} from 'lucide-react'
import {
  deployments as initialDeployments, teams,
  OPERATION_STATUSES, STATUS_LABEL, STATUS_COLOR,
  TEAM_STATUS_COLOR, TEAM_STATUS_LABEL,
  type Deployment, type OperationStatus,
} from '../data/officerData'
import { SEVERITY_HEX, SEVERITY_LABEL } from '../types'
import type { Severity } from '../types'

// ── Relocate Modal ──────────────────────────────────────────
function RelocateModal({ teamId, fromDepId, deployments, onConfirm, onClose }: {
  teamId: string
  fromDepId: string
  ...
  const targets = deployments.filter(d => d.id !== fromDepId && d.status !== 'completed')
  ...
  if (stage === 'approved') {
    return (
      <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 anim-backdrop" style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)' }}>
        <div className="w-full max-w-sm rounded-2xl overflow-hidden anim-scale-up text-center p-8 space-y-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-lg)' }}>
          <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto" style={{ background: '#16a34a18' }}>
      ...
      <div className="w-full max-w-md rounded-2xl overflow-hidden anim-scale-up" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-lg)', maxHeight: '90vh' }}>
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 'calc(90vh - 10rem)' }}>
          <div className="px-5 py-4 space-y-4">
            <ReportField label="Team Lead"       value={r.teamLead} />
            <ReportField label="Completed At"    value={r.completionTime} />
            <ReportField label="People Rescued"  value={String(r.peopleRescued)} highlight />
            <ReportField label="Submitted At"    value={r.submittedAt} />
...
// ── Deployment Card ─────────────────────────────────────────
function DeploymentCard({ dep, index, allDeployments, onUpdate }: {
  dep: Deployment
  index: number
  ...
  const color = SEVERITY_HEX[dep.severity]
  const statusColor = STATUS_COLOR[dep.status]
  const assignedTeams = teams.filter(t => dep.assignedTeamIds.includes(t.id))
  const availableTeams = teams.filter(t => t.status === 'available' && !dep.assignedTeamIds.includes(t.id))

  const handleDeploy = (teamId: string) => {
    onUpdate(dep.id, { assignedTeamIds: [...dep.assignedTeamIds, teamId] })
  }

  const handleRelocate = (teamId: string, toDepId: string) => {
    onUpdate(dep.id, { assignedTeamIds: dep.assignedTeamIds.filter(id => id !== teamId) })
    onUpdate(toDepId, {
      assignedTeamIds: [...(allDeployments.find(d => d.id === toDepId)?.assignedTeamIds ?? []), teamId]
    })
    ...
    >
      ...
      >
        <div>
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-mono mt-0.5" style={{ color: 'var(--fg-muted)' }}>
        ...
        <div className="divide-y anim-fade-up" style={{ borderColor: 'var(--border)' }}>
          ...
          <Section label="Assigned Teams" count={assignedTeams.length}>
            {assignedTeams.length === 0 ? (
              ...
              const tc = TEAM_STATUS_COLOR[t.status]
              return (
          ...
          {dep.status !== 'completed' && availableTeams.length > 0 && (
          ...
          {dep.status !== 'completed' && (
            <div className="px-4 py-3 flex items-center gap-3">
              ...
              >
                {updatingStatus ? <span className="anim-blink">Updating…</span> : 'Update'}
              </button>
            </div>
          )}
      ...
      {showReport && dep.missionReport && (
        <MissionReportModal
          dep={dep}
          onApprove={() => onUpdate(dep.id, { status: 'completed' })}
          onRequestChanges={() => {}}
...
function DeploymentsContent() {
  ...
  const [deps, setDeps] = useState<Deployment[]>(initialDeployments)
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState<OperationStatus | 'all'>('all')
  const [sevFilter, setSevFilter] = useState<Severity | 'all'>('all')

  const handleUpdate = (id: string, patch: Partial<Deployment>) => {
  ...
  const active = deps.filter(d => d.status !== 'completed').length
      ...
      >
        ...
        {alertId && (
          <div
            ...
            style={{ background: '#1a5fc418', border: '1px solid #1a5fc430', color: 'var(--primary)' }}
          >
            <Truck size={10} /> Redirected from Alert
          </div>
        )}
      </div>
